...
def main(args):
    ...
    if True:  # args.distributed:
        ...
        if args.weighted_lgs:
            sampler_train = [torch.utils.data.DistributedSampler(dataset_train[0], num_replicas=num_tasks, 
                                                                 rank=global_rank, shuffle=True),
                             DistributedWeightedSampler(dataset_train[1], num_replicas=num_tasks, 
                                                        rank=global_rank, shuffle=True, replacement=True)]
        else:
            sampler_train = [torch.utils.data.DistributedSampler(ds, num_replicas=num_tasks, 
                                                                 rank=global_rank, shuffle=True)
                             for ds in dataset_train]
        print("Sampler_train = %s" % str(sampler_train))
    else:
        sampler_train = [torch.utils.data.RandomSampler(ds) for ds in dataset_train]
    ...
    data_loader_train = [torch.utils.data.DataLoader(ds, sampler=sa,
                                                     batch_size=args.batch_size,
                                                     num_workers=args.num_workers,
                                                     pin_memory=args.pin_mem,
                                                     collate_fn=lgs_collate,
                                                     drop_last=True)
                         for (ds, sa) in zip(dataset_train, sampler_train)]
    ...
    ebs_fac = 2 if args.use_lgs else 1
    eff_batch_size = args.batch_size * args.accum_iter * misc.get_world_size() * ebs_fac
